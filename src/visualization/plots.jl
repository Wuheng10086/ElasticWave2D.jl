...
Plot survey setup with irregular free surface.

# Arguments
- `model`: VelocityModel
- `z_surface`: Surface elevation array
- `src_x`, `src_depth`: Source positions (x and depth relative to surface)
- `rec_x`, `rec_depth`: Receiver positions
- `output`: Output file path (optional)
- `title`: Plot title
- `show_velocity`: Show velocity model background (default: true)
"""
...
Compare flat and irregular surfaces.
"""
...
Plot shot gather with surface elevation information.
"""
